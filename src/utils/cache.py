...
class ChatCache:
    ...
    def save_auth_data(self, api_key, pin, telegram_chat_id):
        """
        Сохранение данных аутентификации.

        Args:
            api_key (str): API ключ OpenRouter
            pin (str): PIN код для входа
            telegram_chat_id (str): ID чата Telegram
        """
        conn = self.get_connection()
        cursor = conn.cursor()
        
        cursor.execute('''
            INSERT OR REPLACE INTO auth_data (api_key, pin, telegram_chat_id)
            VALUES (?, ?, ?)
        ''', (api_key, pin, telegram_chat_id))
        conn.commit()
    ...
    def get_auth_data(self):
        ...
        Returns:
            tuple: (api_key, pin, telegram_chat_id) или None если данных нет
        """
        conn = self.get_connection()
        cursor = conn.cursor()
        
        cursor.execute('SELECT api_key, pin, telegram_chat_id FROM auth_data LIMIT 1')
    ...
    def verify_pin(self, pin):
        ...
        Args:
            pin (str): PIN код для проверки

        Returns:
            bool: True если PIN верный, False если нет
        """
        conn = self.get_connection()
        cursor = conn.cursor()
        
        cursor.execute('SELECT pin FROM auth_data WHERE pin = ? LIMIT 1', (pin,))
        result = cursor.fetchone()
